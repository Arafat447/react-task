...
const Problem2 = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        await fetch(`https://contact.mediusware.com/api/${dataToFetch === 'all-contacts' ? 'contacts' : 'country-contacts/United%20States'}/?page=${page}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            if (dataToFetch !== undefined) {
                await fetch(`https://contact.mediusware.com/api//${dataToFetch === 'all-contacts' ? 'contacts' : 'country-contacts/United%20States'}/?page=1`)
                    .then(res => {
                        return res.json()
                    })
                    .then(data => {
    ...
    return (
        ...
        <div className="container">
            <div className="row justify-content-center mt-5">
                ...
                <ModalA modalState={modalState} data={data} setContactData={setContactData} checkbox={checkbox} setCheckbox={setCheckbox} isLoading={isLoading} setIsLoading={setIsLoading} dispatch={dispatch} handleModalShowOne={handleModalShowOne} handleModalCloseOne={handleModalCloseOne} ref={modalBodyRef} />


                <Modal show={modalState.showModal2} size="md" centered>
                    <Modal.Header>
                        <button className={`btn btn-lg modal_btn_c`} type="button" onClick={() => dispatch({ type: 'showModalTwo', value: false })}>Close</button>
                    </Modal.Header>
                    <Modal.Body>
